...
class Main_Window(Ui_main_window):
    ...
    def __init__(self, db, trello, google):
        ...
        self.refresh_lock = QtCore.QMutex()
...
########################################################################################################################
    ...
    def connect_all_error_frames(self):
        worker1 = Worker(self.error_frame_hide, self.groups_error_frame)
        #self.groups_close_popup_button.clicked.connect(lambda:self.threadpool.start(worker1))
    ...
    def widgets_refresh(self):
        ...
        self.groups_tree_worker = Worker(self.populate_groups_tree, self.groups_tree, self.groups_model, self.groups_node)
        self.forums_tree_worker = Worker(self.populate_groups_tree, self.forum_tree, self.forum_model, self.forum_node)
        self.docs_tree_worker = Worker(self.populate_groups_tree, self.docs_tree, self.docs_model, self.docs_node)
        self.forums_worker = Worker(self.populate_forum_view)
        self.docs_worker = Worker(self.get_group_documents)
        self.trello_tree_worker = Worker(self.set_trello_tree)
        self.invites_worker = Worker(self.get_invites)


        print("refreshing")
        self.threadpool.start(self.groups_tree_worker)
        #self.populate_groups_tree(self.groups_tree, self.groups_model, self.groups_node)
        print("populated groups_tree")
        self.threadpool.start(self.forums_tree_worker)
        #self.populate_groups_tree(self.forum_tree, self.forum_model, self.forum_node)
        print("populated forum_tree")
        self.threadpool.start(self.docs_tree_worker)
        #self.populate_groups_tree(self.docs_tree, self.docs_model, self.docs_node)
        print("populated docs_tree")
        self.threadpool.start(self.forums_worker)
        #self.populate_forum_view()
        print("populating forums")
        self.threadpool.start(self.docs_worker)
        #self.get_group_documents()
        print("get_group_docs")
        self.threadpool.start(self.trello_tree_worker)
        #self.set_trello_tree()
        print("setting trello tree")
        self.threadpool.start(self.invites_worker)
        #self.get_invites()
        print("getting invites")
        #self.get_months_events()
        #print("getting months events")
        print("starting timer")
        self.refresh_lock.unlock()
    ...
    def set_google_client(self, google):
        self.google_client = google
